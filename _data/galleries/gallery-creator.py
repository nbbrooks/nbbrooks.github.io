...
for f in files:
    name = splitext(f)[0]
    # print(splitext(f)[0] + "-%sx%s" % imagesize.get(join(path, f)) + f[f.rfind('.'):])

    # if f[f.rfind('-')+1:f.rfind('.')] != 'thumbnail':
    #     newf = f[:f.rfind('-')] + "-%sx%s" % imagesize.get(join(path, f)) + f[f.rfind('.'):]

    # if name[index+1:] != 'thumbnail':
    if f[f.rfind('-')+1:f.rfind('.')] != 'thumbnail':
        prefix = name if name.rfind('-') == -1 else name[:name.rfind('-')]
        newf = prefix + "-%sx%s" % imagesize.get(join(path, f)) + f[f.rfind('.'):]
        print(newf)
...
for pic in new_gallery:
    ...
    if not found:
        # create new entry
        if old_gallery == None:
            old_gallery = {"filename": pic, "sizes": new_gallery[pic], "thumbnail": thumbs[pic], "original": originals[pic]}
            print("\t" + str(old_gallery))
        else:
